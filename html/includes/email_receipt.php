...
require_once(BASE_URI.'email.config.php');
$email=new stdClass();
$email->{'personalizations'}=[(object)['to'=>[(object)['email'=> $_SESSION['email'] ]],
'subject'=>"Order #{$_SESSION['order_id']} from Shop Art"]];
$email->{'from'}=(object)['email'=>EMAIL_SENDER_ADDRESS,'name'=>EMAIL_SENDER_NAME];
$email->{'content'}=[(object)['type'=>'text/html','value'=>$body_html]];
$ch = curl_init();
